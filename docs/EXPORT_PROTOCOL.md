...
> - Anything else: any `<name>_Boundary.shp` + `<name>_Rx.shp` pair imports too (e.g. a
>   vineyard's own block codes like `CHRBK04 - Cabernet_Rx.shp`) — block ids fall back to
>   sequential and the name is used as-is. The dry-run preview shows the mapping first.
>
